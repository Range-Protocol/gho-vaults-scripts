...
def process_vault(name, chain, vault, last_block, w3):
    ...
    for block in blocks:
        ...
        prev_total_amount = prev_balance - prev_fee
        ...
        if is_token0_gho:
            ...
            current_fee0 = toInt(current_fee_data[0: 66])
            current_fee1 = toInt(current_fee_data[66: 130]) * 10 ** decimal0 / 10 ** decimal1

        current_fee = current_fee0 + current_fee1
        current_balance = toInt(call(w3, vault, get_underlying_balance_selector, block))
        current_total_amount = current_balance - current_fee

        diff = current_total_amount - prev_total_amount
        amount = amount + (diff / 10 ** decimal1 if is_token0_gho else decimal0)
...
for data in chain_data:
    if len(data["vaults"]) > 0:
        ...
        with open("../uni-analytics/data/aave-arbitrage-fees-{}.json".format(data["name"]), "w") as outfile:
